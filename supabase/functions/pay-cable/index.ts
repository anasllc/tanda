// supabase/functions/pay-cable/index.ts
...
serve(async (req) => {
  ...
  try {
    ...
    const errors = validate(body, [
      { field: "smartcard_number", type: "string", required: true },
      { field: "amount_local", type: "number", required: true, min: 100000 },
      { field: "provider", type: "string", required: true, enum: ["DSTV", "GOTV", "STARTIMES", "SHOWMAX"] },
      { field: "package_id", type: "string", required: true },
    ...
    const { data: billPayment, error: billError } = await supabase
      ...
      .insert({
        ...
        recipient_identifier: body.smartcard_number,
    ...
    const { data: tx, error: txError } = await supabase
      ...
      .insert({
        ...
        fee_usdc: fee,
        memo: `${body.provider} - ${body.smartcard_number}`,
    ...
    // TODO: Call actual cable provider API
    ...
    await supabase
      ...
      .upsert({
        ...
        provider: body.provider,
        recipient_identifier: body.smartcard_number,
        label: body.label || `${body.provider} - ${body.smartcard_number}`,
