// supabase/functions/buy-airtime/index.ts
...
serve(async (req) => {
  ...
  try {
    ...
    const errors = validate(body, [
      ...
      { field: "amount_local", type: "number", required: true, min: 5000 }, // min ₦50 in kobo
    ...
    // Get exchange rate for NGN → USDC conversion
    ...
    const amountUsdc = Math.ceil((body.amount_local / 100) / exchangeRate * 1_000_000); // kobo → NGN → USDC
    ...
    const { data: billPayment, error: billError } = await supabase
      ...
      .insert({
        ...
        local_currency: "NGN",
        status: "processing",
        metadata: { network: body.provider },
      })
      .select()
      .single();
    ...
    const { data: tx, error: txError } = await supabase
      ...
      .insert({
        ...
        fee_usdc: fee,
        memo: `${body.provider} airtime for ${phone}`,
    ...
    // Save to saved_bill_recipients for quick reorder
    await supabase
    ...
    // TODO: Call actual airtime provider API (VTPass, Flutterwave, etc.)

    const response = {
      bill_payment: billPayment,
      transaction: tx,
      fee_usdc: fee,
      amount_ngn: body.amount_local / 100,
    };
